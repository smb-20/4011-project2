...
st.markdown(
    ...
    **Terminal value check:**  
    Terminal value uses the cash flow **one year after** the final forecast year.  
    Next year FCFF = {money(final_fcff)} × (1 + {percent(terminal_growth)}) = **{money(next_year_fcff)}**  
    Terminal value = {money(next_year_fcff)} ÷ ({percent(wacc)} − {percent(terminal_growth)}) = **{money(terminal_value)}**
    """
)
